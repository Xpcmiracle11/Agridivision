...
if (isset($_POST['fname'])) {
    ...
    if (!empty($_FILES['profileimg']['name'])) {
        ...
        if (!in_array($imageFileType, $allowedFormats)) {
            ...
            $destinationPath = "C:/xampp/htdocs/AGRIDIVISION_FINAL/assets/img/brgy_tech/";
            ...
            if (move_uploaded_file($_FILES['profileimg']['tmp_name'], $destinationFile)) {
                // Update the brgy_tech record in the database
                $user_id = $_SESSION['user_id'];
                $sql = "UPDATE brgy_tech SET fname='$fname', lname='$lname', email='$email', num='$num', prov='$prov', mun='$mun', brgy='$brgy', street='$street', zip='$zip', dob='$dob', password='$password', profileimg='$newImageNameWithoutExtension'  WHERE id=$user_id";
    
                if (mysqli_query($conn, $sql)) {
                    $alertMessage = "Barangay Technician profile has been updated";
                } else {
                    $alertMessage = "Error updating Barangay Technician profile: " . mysqli_error($conn);
                }
            } else {
        ...
        // Update the brgy_tech record in the database without changing the image
        $user_id = $_SESSION['user_id'];
        $sql = "UPDATE brgy_tech SET fname='$fname', lname='$lname', email='$email', num='$num', prov='$prov', mun='$mun', brgy='$brgy', street='$street', zip='$zip', dob='$dob', password='$password' WHERE id=$user_id";
...
// Retrieve data from the "brgy_tech" table
if (isset($_SESSION['user_id'])) {
    $user_id = $_SESSION['user_id'];
    $sql = "SELECT * FROM brgy_tech WHERE id = $user_id";
    ...
    function handleModalClose() {
        ...
        switch (resultMessage) {
            case "Passwords do not match":
                window.location.replace("techprofileedit.php");
                break;
            case "Barangay Technician profile has been updated":
                window.location.replace("techprofile.php");
                break;
            default:
