...
if (isset($_POST['fname'])) {
    ...
    if (!empty($_FILES['profileimg']['name'])) {
        ...
        if (!in_array($imageFileType, $allowedFormats)) {
            ...
            $destinationPath = "C:/xampp/htdocs/AGRIDIVISION_FINAL/assets/img/municipal_agriculturist/";
            ...
            if (move_uploaded_file($_FILES['profileimg']['tmp_name'], $destinationFile)) {
                // Update the municipal_agriculturist record in the database
                $user_id = $_SESSION['user_id'];
                $sql = "UPDATE municipal_agriculturist SET fname='$fname', lname='$lname', email='$email', num='$num', prov='$prov', mun='$mun', brgy='$brgy', street='$street', zip='$zip', dob='$dob', password='$password', profileimg='$newImageNameWithoutExtension'  WHERE id=$user_id";
    
                if (mysqli_query($conn, $sql)) {
                    $alertMessage = "Municipal Agriculturist profile has been updated";
                } else {
                    $alertMessage = "Error updating Municipal Agriculturist profile: " . mysqli_error($conn);
                }
            } else {
        ...
        // Update the municipal_agriculturist record in the database without changing the image
        $user_id = $_SESSION['user_id'];
        $sql = "UPDATE municipal_agriculturist SET fname='$fname', lname='$lname', email='$email', num='$num', prov='$prov', mun='$mun', brgy='$brgy', street='$street', zip='$zip', dob='$dob', password='$password' WHERE id=$user_id";
...
// Retrieve data from the "municipal_agriculturist" table
if (isset($_SESSION['user_id'])) {
    $user_id = $_SESSION['user_id'];
    $sql = "SELECT * FROM municipal_agriculturist WHERE id = $user_id";
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        include 'municipalprofileedit.html'; 
    }
}
?>
    ...
    function handleModalClose() {
        ...
        switch (resultMessage) {
            case "Passwords do not match":
                window.location.replace("municipalprofileedit.php");
                break;
            case "Municipal Agriculturist profile has been updated":
                window.location.replace("municipalprofile.php");
                break;
            default:
