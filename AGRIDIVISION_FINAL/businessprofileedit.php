...
if (isset($_POST['fname'])) {
    ...
    $buspermit = mysqli_real_escape_string($conn, $_POST['buspermit']);
    ...
    if (!empty($_FILES['profileimg']['name'])) {
        ...
        if (!in_array($imageFileType, $allowedFormats)) {
            ...
            $destinationPath = "C:/xampp/htdocs/AGRIDIVISION_FINAL/assets/img/business_owner/";
            ...
            if (move_uploaded_file($_FILES['profileimg']['tmp_name'], $destinationFile)) {
                // Update the business_owner record in the database
                $user_id = $_SESSION['user_id'];
                $sql = "UPDATE business_owner SET fname='$fname', lname='$lname', email='$email', num='$num', busspermit='$buspermit', crop='$crop', prov='$prov', mun='$mun', brgy='$brgy', street='$street', zip='$zip', dob='$dob', password='$password', profileimg='$newImageNameWithoutExtension'  WHERE id=$user_id";
    
                if (mysqli_query($conn, $sql)) {
                    $alertMessage = "Business Owner profile has been updated";
                } else {
                    $alertMessage = "Error updating Business Owner profile: " . mysqli_error($conn);
                }
            } else {
        ...
        // Update the business_owner record in the database without changing the image
        $user_id = $_SESSION['user_id'];
        $sql = "UPDATE business_owner SET fname='$fname', lname='$lname', email='$email', num='$num', busspermit='$buspermit', crop='$crop', prov='$prov', mun='$mun', brgy='$brgy', street='$street', zip='$zip', dob='$dob', password='$password' WHERE id=$user_id";
...
// Retrieve data from the "business_owner" table
if (isset($_SESSION['user_id'])) {
    $user_id = $_SESSION['user_id'];
    $sql = "SELECT * FROM business_owner WHERE id = $user_id";
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        include 'businessprofileedit.html'; 
    }
}
?>
...
<script>
    ...
    document.getElementById('buspermit').value = '<?php echo isset($buspermit) ? $buspermit : ""; ?>';
    ...
    function handleModalClose() {
        ...
        switch (resultMessage) {
            case "Passwords do not match":
                window.location.replace("businessprofileedit.php");
                break;
            case "Business Owner profile has been updated":
                window.location.replace("businessprofile.php");
                break;
            default:
